...
export const PlayerComparison: React.FC<PlayerComparisonProps> = ({ 
  ...
  // Filter available prospects of the same position for candidate lists
    ...
    try {
      ...
      categories.forEach((cat: any) => {
        if (cat.name === 'general') {
          ...
          // Forced Fumbles mapping as requested
  ...
  const renderComparisonRow = (label: string, key1: any, key2: any, lowerIsBetter = false, skipHighlight = false) => (
    <div className="grid grid-cols-3 border-b border-slate-800/50 py-3 lg:py-4 px-4">
      <div className={`text-center font-oswald text-sm lg:text-lg ${skipHighlight ? 'text-slate-300' : getWinnerClass(key1, key2, lowerIsBetter)}`}>
        {key1?.toLocaleString() || '--'}
      </div>
      ...
      <div className={`text-center font-oswald text-sm lg:text-lg ${skipHighlight ? 'text-slate-300' : getWinnerClass(key2, key1, lowerIsBetter)}`}>
        {key2?.toLocaleString() || '--'}
      </div>
    </div>
  );
        ...
        <div className="flex-1 overflow-y-auto bg-slate-900/50">
          {!player2 ? (
            ...
            <div className="divide-y divide-slate-800/50 pb-10">
              ...
              {renderComparisonRow('Overall Rank', player1.rank, player2.rank, true)}
              {renderComparisonRow('Height', player1.height, player2.height)}
              {renderComparisonRow('Weight', player1.weight, player2.weight, false, true)}
                ...
                <>
                  ...
                  {player1.position === 'QB' && (
                    <>
                      ...
                      {renderComparisonRow('INTs', stats1?.ints, stats2?.ints, true)}
